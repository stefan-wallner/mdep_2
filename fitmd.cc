...
std::string W= "../wave_specifications.yaml";
std::string P="../parametrizations_13_waves.yaml";
				...
				if (fMap[fName]==0){															//
					...
					if (nParDef == nPar){														//
						for(int par=0;par<nPar;par++){												//
							...
							pRels.push_back(rel);												// < was never used
...
// Define waves																				//
	...
	for (int i=0;i<nWaves;i++){																	//
		...
		Chi2.setWaveSpin(i,defs[wName]["spin"].as<int>());													// // Also load isobar spin
		...
		for (int func=0;func<nFunc;func++){															//
			...
			int nf = fMap[fName]-1;																//
			Chi2.add_func_to_wave(i,nf);															// // Also include isobars
		};																			//
	};																				//
// Set up branchings																			//
...
//Set up data																				//
	...
	Chi2.update_min_max_bin(); //Has to be called to set internal definitions right, since not all belong to the same class now					//
